"""Default secrets so importing `nakabandi.main` (which builds a module-level `app` via
`create_app()`, the uvicorn `module:variable` convention for Step A5) never fails Settings
...
os.environ.setdefault("NAKABANDI_TIMER_WORKER_ENABLED", "false")  # tests call run_timers_once
