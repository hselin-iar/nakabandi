...
[P6/global-density] -- Implemented `global_cashout_count` + `global_cashout_rate` (Laplace/EB smoothed, alpha=5) in FeatureRow/FEATURE_REGISTRY. New pure domain class GlobalCashoutIndex (forecast/domain/global_stats.py) replays a flat (location_id, observed_at) feed into as-of snapshots -- same replay pattern as PointInTimeStats but unscoped by cluster. New infra method LienContextLookup.all_cashout_events() supplies the raw feed (one query). Wired into both SqlTrainingDataPort._forecast_ctx (training/eval) and LiveClusterPort.context_for (live serving) so candidate generation and scoring see consistent values. features_to_array is now (n,15); channel moved to column 14. HeuristicScorer got two new defensively-NaN-guarded weight terms. All existing tests updated (shape/column-index) plus 13 new focused tests (GlobalCashoutIndex zero/single/multi-location/future-exclusion/order-independence; global_cashout_rate formula/cold-start/empty-universe; build_features wiring). npm run ci:py: 685/686 pytest green, 19/19 import-linter contracts kept, 0 pyright errors, ruff clean. The 1 failure (test_recovery.py kill-9 webhook-delivery-status race) is pre-existing flakiness unrelated to this change -- confirmed by re-running in isolation (different assertion value each run) and by diffing untouched files (candidates.py, policy.yaml).

[P6/eval-blocked] -- Retrained (scorer_id 01M3EFJXB9G3YMG8W8QPQ8K96A) and re-ran scripts/evaluate.py against DATABASE_URL=sqlite:///nakabandi.db: only 17 complaints now pass the candidate-radius match filter (SqlTrainingDataPort.complaint_records), down from the 951 this doc recorded two commits ago. DB currently holds 882 complaints / 3318 cashouts at latest_ingest_sim_time=2026-01-17T10:15:38Z -- fewer than whatever state produced 951. World-sim is not a persistent daemon here; nobody kept it advancing between sessions, so the historical volume the prior comparison relied on isn't reproducible from this DB as-is. n=17 trips metrics.py's n<30 guard (Hit@k/Precision@k -> NaN for every scorer, hgb and baselines alike); an informational unguarded check (same n=17) shows hgb_v2/hotspot/nearest/bank_footprint all within one hit of each other -- not a usable signal either way. New scripts/evaluate_baselines.py is ready to produce the full leakage-free (per-complaint as-of) comparison table the moment there's a large-enough resolved-complaint set again (e.g. after running world-sim/live ingestion longer). Flagged to user rather than reporting a fabricated "hgb beats/loses to hotspot" conclusion from n=17.



[A/inject-cluster] -- Change made by the Integration Owner in Track B's files (bug found in a live demo run). `scenarios.inject_cluster` created clusters with `accounts=[]` and ignored `size`; the generator then gave each complaint an innocent `INNO-` layer-1 account and routed all hops back to it (self-loops, one single-node cluster per complaint downstream). Fix: inject_cluster builds max(2, size) accounts (random bank, home location in the district, ids `ACC-<cluster_id>-NNNN`); `World.step` only routes complaints to clusters that have accounts. Tests: apps/world-sim/tests/unit/test_inject_cluster.py (new); `_fake_world` in test_live_runner.py now supplies registry.banks/locations_in. Golden hash is unaffected (default world has no injected clusters). B-side follow-up: merge feat/track-a into feat/track-b before touching core/scenarios.py or core/generator.py.
